...
// advanced preview element
const previewPopup = document.createElement('div');
previewPopup.className = 'preview-popup';
previewPopup.innerHTML = '<img src="" alt=""><div class="info"><h4></h4><p class="price"></p></div>';
...
function showPreview(card, x, y) {
    ...
    const titleEl = previewPopup.querySelector('h4');
    const priceEl = previewPopup.querySelector('.price');
    ...
    previewPopup.style.top = `${y + 20}px`;
...
function hidePreview() {
    ...
    document.querySelectorAll('.product-card.hovered').forEach(c => c.classList.remove('hovered'));
}

// proximity detection (only on devices that support hover)
const cards = document.querySelectorAll('.product-card');
if (window.matchMedia('(hover: hover)').matches) {
    document.addEventListener('mousemove', (e) => {
        let closest = null;
        let minDist = 150; // px threshold
        cards.forEach(card => {
        ...
        if (closest) {
            showPreview(closest, e.clientX, e.clientY);
        } else {
            hidePreview();
        }
    });
